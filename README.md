...
1. **BDD-кейсы не создаются через API**: `POST /testcase` с `testScript.type = "BDD"` возвращает `400 {"errorMessages":["Invalid BDD Script"]}` на любой корректный Gherkin (проверено: с `Feature:` и без, кириллица и латиница). STEP_BY_STEP и PLAIN_TEXT работают. Вероятно, версия плагина не поддерживает BDD через REST или BDD выключен для проекта.
